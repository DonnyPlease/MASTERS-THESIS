...
if __name__ == "__main__":
    ...
    x_pred = np.array([[i, 1, 0] for i in i_space])
    i_space = np.logspace(16.9, 19.1, 50)
    x_ss = np.array([[i, 1, 0] for i in i_space])
    ...
    popt, pcov = curve_fit(power, x_pred[:, 0], t_pred, p0=[1, 1/3])
    ...
    plt.fill_between(x_ss[:, 0], mean + ss, mean - ss, color='red', alpha=0.2, label="GP standard error")
    plt.xscale("log")
    plt.yscale("log")
    ...
    label = r"$b$ = {:.3f} ± {:.3f} keV".format(popt[1], std_popt[1])
    labels.append(label)
    plt.legend(handles=handles, labels=labels)

    # Set x-axis limits and show the plot
    plt.xlim(10**16.9, 10**19.1)
    plt.savefig("graphs_for_thesis/comparison/cui2013_4.pdf")
    plt.show()
