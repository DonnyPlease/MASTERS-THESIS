...
from fitting.helpful_functions import load_folder_names_from_params_file, load_histogram
...
from fitting.reduce_historgram import cut_histogram_from_left
...
if __name__ == '__main__':
    folder_names = load_folder_names_from_params_file("old_data/params.txt")
    print(folder_names[310])
    bins, counts = load_histogram("old_data/moved_histograms/trimmed_histograms/"+folder_names[310])
    bins, counts = cut_histogram_from_left(bins, counts, len(bins)//10)
    ...
    optimizer.optimize_double_exp(initial_guess=[0.2,0.02])
   ...
   # plot the residuals
    plt.scatter(bins, counts-optimizer.get_y_pred(bins))
    plt.show() 
