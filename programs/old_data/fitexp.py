...
class FitExp():
    ...
    def _calculate_integrals(self):
        ...
        for i in range(self.exp_count):  # For each exponential function 
                                         ...
                                         # intergral from the previous integral.
            S = np.zeros(self.N)
    ...
    def _calculate_linearized_vector(self, integrals):
        """This function is responsible for creating a vector that later plays the role of the data vector that is fitted using ordinary least squares.
        ...
        if self.constant: size_of_vector += 1  # add one to the size if constant is also fitted
    ...
    def fit(self, verbose=False):
        ...
        desrcibed in the article of Mr. Jean Jaquelin.
        ...
        b_2 = np.dot(np.linalg.inv(np.matmul(G, np.transpose(G))),np.matmul(G, self.y))
    ...
    def plot(self, log=False, residuals=False, show=True, save=False, save_name=None):
        y_fit = self.predict()
        res = self.y - y_fit
        
        fig, (ax1,ax2) = plt.subplots(2,1,sharex=False, gridspec_kw={'height_ratios': [4, 1]}, figsize=(8,6)) 
        ax1.scatter(self.x, self.y)
        ax1.plot(self.x, y_fit, c='r')
        ax1.set_xlabel('E [keV]')
        ax1.set_ylabel('Electron counts')
        ax1.set_title('{}-exponential fit'.format(self.exp_count))
        if log: 
            ax1.set_yscale('log')
            ax1.set_title('{}-exponential fit (log scale)'.format(self.exp_count))
