...
if __name__ == "__main__":
    ...
    i_space = np.logspace(17, 19, 20)
    x_pred = np.array([[i, 1, 45] for i in i_space])
    i_space = np.logspace(16.9, 19.1, 50)
    x_ss = np.array([[i, 1, 45] for i in i_space])
    ...
    fit_x = np.array([x_pred[:, 0][0], x_pred[:, 0][-1]])
    fit_t = np.array([t_pred[0], t_pred[-1]])
    # Fit the power-law curve
    popt, pcov = curve_fit(power, fit_x, fit_t, p0=[1])
    ...
    fitted_y = power(fitted_x, popt[0])

    # Plotting
    plt.plot(fitted_x, fitted_y, color='black', label=r"$T_{\mathrm{hot}} \sim I^{0.64}$",zorder=8)
    plt.scatter(x_pred[:, 0], t_pred, color='red', label="GP predictions", zorder=7)
    plt.fill_between(x_ss[:, 0], mean + ss, mean - ss, color='red', alpha=0.2, label=r"Prediction 95 $\%$ confidence interval")
    plt.xscale("log")
    plt.yscale("log")
    plt.yticks([1e1,2e1,5e1,1e2, 2e2, 5e2], ["10", "20", "50", "100", "200", "500"])
    ...
    plt.savefig(PATH_TO_FIGURES+"cui_compare2.pdf")
    plt.show()
