...
def _fit_using_three_exponentials(histogram):
    ...
    try:
        ...
        fit_results["t_hot"], fit_results["n_hot"], _, _ = get_t_hot_and_n_hot(fit_results)
...
def _fit_using_two_exponentials(histogram):
    ...
    try:
        ...
        fit_results["b1_stdev"], fit_results["b2_stdev"] = errors[0], errors[1]
        fit_results["t_hot"], fit_results["n_hot"], _, _ = get_t_hot_and_n_hot(fit_results)
...
def _fit_using_nlsq(histogram, initial_guess):
    x, y = histogram[0], histogram[1]
    popt, pcov = curve_fit(exp2, x, y, p0=initial_guess)
    return popt, pcov
...
def plot_fit(histogram, fits_params, fit_type):
    match fit_type:
        case "3exp":
            if "3exp" not in fits_params or fits_params["3exp"] is None:
                ...
                return
            _plot_fit_3exp(histogram, fits_params["3exp"])
        case "2exp":
            if "2exp" not in fits_params or fits_params["2exp"] is None:
                ...
                return
            _plot_fit_2exp(histogram, fits_params["2exp"])
        case "nlsq":
            if "nlsq" not in fits_params or fits_params["nlsq"] is None:
                ...
                return
            _plot_fit_nlsq(histogram, fits_params["nlsq"])

def _plot_fit_3exp(histogram, fit_params):
    plt.clf()
    ...
    plt.savefig(PATH_TO_PROJECT+"histograms/data/fitting_results/3exp_plots/"+histogram[2]["filename"]+".pdf")
    
def _plot_fit_2exp(histogram, fit_params):
    plt.clf()
    ...
    _add_t_hot_to_legend(fit_params["t_hot"], None)
    plt.grid(True, zorder=0)
    plt.savefig(PATH_TO_PROJECT+"histograms/data/fitting_results/2exp_plots/"+histogram[2]["filename"]+".pdf")

def _plot_fit_nlsq(histogram, fit_params):
    plt.clf()
    ...
    _add_t_hot_to_legend(fit_params["t_hot"], None)
    plt.grid(True, zorder=0)
    plt.savefig(PATH_TO_PROJECT+"histograms/data/fitting_results/nlsq_plots/"+histogram[2]["filename"]+".pdf")
...
def get_t_hot_and_n_hot(fit_params, std_errors=None):
    ...
    std_t_hot = std_errors["b"+str(index_of_max+1)]/(max_number**2)
    std_n_hot = std_errors["a"+str(index_of_max+1)]
...
def save_fit_results(fit_results):
    json.dump(fit_results, open(PATH_TO_PROJECT+f"histograms/data/fitting_results/{fit_results["histogram"]["filename"]}.json", "w"), indent=4)
...
def fit_hot_temperature(histogram):
    ...
    try:
        ...
        final_params["nlsq_std_errors"] = fit_stdev
    except:
